...
<c:set var="articlesList" value="${resourcesMap.resourcesList}" />
<c:set var="totArticles" value="${resourcesMap.totResources}" />
							...
							<c:choose>
								...
								<c:when test="${resource.level > 1 }">
								  ...
								  --%>
									...
									<a class="cls1"
										href="${path }/경로.bo?res_no=${resource.res_no}">${resource.res_title }</a>
								</c:when>
				...
				<c:when test="${totResources >100 }">
					...
					<c:forEach var="page" begin="1" end="10" step="1">
						...
						<c:if test="${section >1 && page==1 }">
							<a class="no-uline"
								href="${path }/listArticles.do?section=${section-1}&pageNum=${(section-1)*10 +1 }">&nbsp;
								pre </a>
						</c:if>

						<a class="no-uline"
							href="${path }/resourceList.bo?section=${section}&pageNum=${page}">${(section-1)*10 +page }
						</a>

						<%--페이지번호 10 오른쪾에는 다음섹션으로 이동할수 있는 next를 표시합니다.--%>
						<c:if test="${page ==10 }">
							<a class="no-uline"
								href="${path }/board8/resourceList.bo?section=${section+1}&pageNum=${section*10+1}">&nbsp;
								next</a>
						</c:if>
					</c:forEach>
				</c:when>
					...
					<%--
	         	 	...
	         	 	--%>
							<c:when test="${page==pageNum }">
								<a class="sel-page"
									href="${path }/resourceList.bo?section=${section}&pageNum=${page}">${page }
								</a>
							</c:when>
							...
							<c:otherwise>
								<a class="no-uline"
									href="${contextPath }/resourceList.bo?section=${section}&pageNum=${page}">${page }
								</a>
							</c:otherwise>
